...
3. 对象之间如何形成 `需求 -> 项目 -> 执行 -> 数据 -> 知识 -> 管理` 的主线关系。
...
| 业务主对象 | 承载业务核心语义的对象 | Requirement、Project、WorkPackage、Task、Test、Contract |
...
5. 低/无代码只能扩展被授权的对象白名单，不能侵入统一安全和谱系模型。
6. `IndustryTemplatePackage / InitializationPlan / AutomationRegistration / PersistenceProfile` 应作为复制引擎的正式对象进入服务设计。
